...
Контракты сгруппированы по лабораторной и задаче. Предметные критерии берутся
из формулировок соответствующих задач методички; неуказанные размеры, стили и
правила разрешения неоднозначных результатов не должны становиться
блокирующими проверками:
...
В проверках computed CSS одиночное значение `font-family`, например `Arial`,
требует это семейство первым в списке и допускает резервные семейства после
него (`Arial, Helvetica, sans-serif`). Кавычки и регистр имени не влияют на
сравнение. Явно заданный список семейств и остальные CSS-свойства сравниваются
целиком. Stylelint допускает одиночный `Arial`, поскольку именно это семейство
указано в УМК (варианты 7, 13, 27 и 33, задача 1). Резервные семейства разрешены,
но для Arial не обязательны. Для остальных имён шрифтов правило Stylelint о
generic family сохраняется.

## Связь с отчётом прогресса

Генератор progress report не выполняет контракты из этого каталога. Он считает
лабораторную принятой, когда её каталог находится в основной ветке курса и
содержит обязательные файлы. Отсутствующий каталог означает несданную работу;
существующий каталог с неполной структурой приводит к предупреждению. Подробная
инструкция по построению отчёта находится в
[docs/USAGE.md](../../docs/USAGE.md#13-progress-report).
