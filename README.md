...
workflow вызывают закреплённый commit utils. Для React-lab5 manifest описывает
Vite/Webpack-проект, но grader собирает JSX/TSX самостоятельно доверенным
esbuild.
...
вариантов lab5 проверяются React-структура приложения и ключевой пользовательский
сценарий конкретной задачи: сортировка, формы, игры, localStorage, drag-and-drop
и другие взаимодействия. Поддерживаются JavaScript/TypeScript и проекты Vite
или Webpack. Выбранный контракт и связь варианта с задачей валидируются
непосредственно во время сдачи.
